...
class DocumentationValidator {
  ...
  async validateAll() {
    ...
    return { issues: this.issues, warnings: this.warnings, passed: this.issues.length === 0 }
  }
  ...
  async runMarkdownlint() {
    ...
    try {
      ...
      try {
        ...
        console.log('ℹ️ markdownlint-cli2 not found - install for enhanced markdown validation')
        return
      }
        ...
        if (output) {
          ...
          const errorLines = lines.filter(line => {
            ...
            return trimmed &&
                   ...
                   (trimmed.includes(':') || trimmed.includes('MD')) // Actual errors have line:column or MD rule format
          })
  ...
  async validateBasicStructure() {
    ...
    if (fs.existsSync('package.json')) {
      if (!readmeLower.includes('npm install') && !readmeLower.includes('pnpm install') && !readmeLower.includes('yarn install')) {
        this.warnings.push('README.md should include package manager installation instructions')
      }
    }
  }
  ...
  async validatePackageJsonAlignment() {
    ...
    if (!packageJson.description || packageJson.description.trim().length === 0) {
    ...
    if (packageJson.name && packageJson.name !== 'my-project' && packageJson.version && (!packageJson.keywords || packageJson.keywords.length === 0)) {
      this.warnings.push(
