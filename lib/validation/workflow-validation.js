...
class WorkflowValidator {
  ...
  async runActionlint() {
    ...
    // Prefer local binary to avoid npx resolution failures in sandboxed/CI environments
    const localActionlint = path.join(
      process.cwd(),
      'node_modules',
      '.bin',
      process.platform === 'win32' ? 'actionlint.cmd' : 'actionlint'
    )
    const actionlintCommand = fs.existsSync(localActionlint)
      ? `"${localActionlint}"`
      : 'npx --yes actionlint'
    ...
    try {
      execSync(actionlintCommand, { stdio: 'pipe', cwd: process.cwd() })
