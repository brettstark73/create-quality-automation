...
class WorkflowValidator {
  ...
  async runActionlint() {
    ...
    try {
      ...
      // Actionlint not installed, skip with info message
      console.log('ℹ️ actionlint not found - install for enhanced workflow validation')
      return
    }

    try {
      // Run actionlint on workflow files
      execSync('actionlint', { stdio: 'pipe', cwd: process.cwd() })
    } catch (error) {
  ...
  async validateWorkflowSyntax() {
    ...
    for (const file of workflowFiles) {
      ...
      try {
        ...
        if (content.includes('ubuntu-latest') && content.includes('node-version:')) {
          // This is likely a Node.js workflow, check for proper setup
          if (!content.includes('actions/setup-node@')) {
            this.issues.push(`${file}: Node.js workflow should use actions/setup-node`)
          }
        }

        // Check for security best practices
        if (content.includes('${{') && content.includes('github.event.pull_request.head.repo.full_name')) {
          this.issues.push(`${file}: Potential security risk using untrusted PR data`)
        }
