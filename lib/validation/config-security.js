...
class ConfigSecurityScanner {
  ...
  async runGitleaks() {
    try {
      ...
      // Gitleaks not installed, skip with info message
      console.log('ℹ️ gitleaks not found - install for enhanced secret detection')
      return
    }

    try {
      // Run gitleaks to detect secrets in the entire repository
      ...
      if (error.status === 1) {
        ...
        if (output.includes('leaks found')) {
          this.issues.push('gitleaks: Potential secrets detected in repository. Run "gitleaks detect" for details.')
        }
      }
      // Exit code 2+ indicates other errors, which we'll ignore
    }
  }
  ...
  async runESLintSecurity() {
    ...
    try {
      ...
      execSync(`npx eslint . --ext .js,.jsx,.ts,.tsx --quiet --config ${eslintConfig}`, {
        stdio: 'pipe',
        encoding: 'utf8'
      })
      ...
      if (output && output.trim()) {
        ...
        lines.forEach(line => {
          // Look for security rules (case-insensitive)
          if (line.toLowerCase().includes('security')) {
            this.issues.push(`ESLint Security: ${line.trim()}`)
          }
        })
      }
    }
  ...
  async checkGitignore() {
    if (!fs.existsSync('.gitignore')) {
      this.issues.push('No .gitignore found. Create one to prevent committing sensitive files.')
      return
    }
