    ...
    try {
      ...
      if (error.status === 1) {
        ...
        if (output.includes('leaks found') || output.includes('Finding:')) {
          ...
          // Read and log the report for debugging
          try {
            const fs = require('fs')
            const report = fs.readFileSync('/tmp/gitleaks-report.json', 'utf8')
            const findings = JSON.parse(report)
            console.log('\n📋 Gitleaks findings:')
            findings.forEach((finding, i) => {
              console.log(
                `  ${i + 1}. ${finding.Commit.substring(0, 7)}:${finding.File}:${finding.RuleID}:${finding.StartLine}`
              )
            })
          } catch (_reportError) {
