...
class ConfigSecurityScanner {
  ...
  async runGitleaks() {
    try {
      ...
      execSync('npx gitleaks detect --source . --redact', {
        stdio: 'pipe',
      ...
      if (error.status === 1) {
        ...
        if (output.includes('leaks found') || output.includes('Finding:')) {
          ...
          this.issues.push(
            `gitleaks: ${leakCount} potential secret(s) detected in repository. Run "npx gitleaks detect --redact" for redacted details.`
          )
        }
      } else {
