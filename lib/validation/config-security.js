...
class ConfigSecurityScanner {
  ...
  async resolveGitleaksBinary() {
    ...
    try {
      ...
      // Log for debugging but don't fail - this is an expected fallback path
      if (process.env.QAA_DEBUG || process.env.NODE_ENV === 'test') {
        console.log(
          `Debug: Global gitleaks not found (${error.code || error.message})`
        )
      }
    }
    ...
    try {
      ...
      if (error.stdout) {
        try {
          ...
          this.issues.push(
            `${packageManager} audit: Unable to parse audit output (${parseError.message}). Run '${packageManager} audit --json' manually to inspect vulnerabilities.`
          )
        }
      } else {
  ...
  async runGitleaks() {
    ...
    try {
      ...
      if (result.error) {
        /** @type {Error & { stdout?: string, stderr?: string, status?: number | null, signal?: string | null }} */
