...
function validateLicenseKey(key, tier) {
  ...
  const stripeFormat = /^QAA-[A-Z0-9]{4}-[A-Z0-9]{4}-[A-Z0-9]{4}-[A-Z0-9]{4}$/

  if (stripeFormat.test(normalizedKey)) {
...
function verifyLicenseSignature(payload, signature) {
  try {
    // In production, LICENSE_SIGNING_SECRET must be set
    // Dev fallback only allowed when QAA_DEVELOPER=true (test mode)
    const secret = process.env.LICENSE_SIGNING_SECRET
    if (!secret) {
      if (process.env.QAA_DEVELOPER === 'true') {
        // Only allow dev secret in test mode
        return true // Skip signature check in dev mode
      }
