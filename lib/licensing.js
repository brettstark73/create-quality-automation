...
function isDeveloperMode() {
  ...
  try {
    ...
    // Silent failure fix: Log security-relevant errors
    if (error?.code === 'ELOOP') {
      console.warn(
        '⚠️  Symlink loop in license directory - possible security issue'
      )
...
function loadUsage() {
  try {
    ...
    // Silent failure fix: Log corruption or unexpected errors
    if (error instanceof SyntaxError) {
      console.warn('⚠️  Usage tracking file corrupted, resetting counters')
    } else if (process.env.DEBUG && error?.code !== 'ENOENT') {
