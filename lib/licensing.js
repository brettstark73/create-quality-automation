...
const FEATURES = {
  [LICENSE_TIERS.FREE]: {
    ...
    roadmap: [
      ...
      '⚠️ Limited: 1 private repo, 2k LOC, JS/TS only',
...
function showUpgradeMessage(feature) {
  ...
  if (license.tier === LICENSE_TIERS.FREE) {
    ...
    console.log(
      '🔑 Activate: npx create-quality-automation@latest --activate-license'
    )
    ...
    if (fs.existsSync(usageFile)) {
      ...
      if (data.month !== currentMonth) {
        ...
        return {
          ...
          dependencyPRs: 0,
          repos: data.repos || [],
          totalLinesOfCode: data.totalLinesOfCode || 0,
        }
      }
...
function checkUsageCaps(operation = 'general') {
  ...
  const result = {
    ...
    usage: {
      ...
      totalLinesOfCode: usage.totalLinesOfCode,
    },
    caps: {
      ...
      maxLinesOfCode: caps.maxLinesOfCode,
    },
  }
  ...
  } else if (operation === 'loc') {
    if (usage.totalLinesOfCode >= caps.maxLinesOfCode) {
      result.allowed = false
      result.reason = `FREE tier limit reached: ${usage.totalLinesOfCode.toLocaleString()}/${caps.maxLinesOfCode.toLocaleString()} lines of code`
    }
  }
...
function incrementUsage(operation, amount = 1, repoId = null) {
  ...
  if (operation === 'pre-push') {
    ...
    usage.totalLinesOfCode = amount // Set to current LOC count, not increment
  }
...
function getUsageSummary() {
  ...
  return {
    ...
    linesOfCode: {
      used: usage.totalLinesOfCode,
      limit: caps.maxLinesOfCode,
      remaining: Math.max(0, caps.maxLinesOfCode - usage.totalLinesOfCode),
    },
  }
}
...
function showLicenseStatus() {
  ...
  if (license.tier === LICENSE_TIERS.FREE) {
    ...
    console.log(
      `   Lines of Code: ${usage.linesOfCode.used.toLocaleString()}/${usage.linesOfCode.limit.toLocaleString()}`
    )
  } else {
    console.log(`   Repos/LOC/Runs: Unlimited`)
  }
