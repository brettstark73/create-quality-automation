...
const distPaths = ['dist', 'build', 'out', '.next', 'public'];
let servePath = null;
...
try {
  ...
  execSync('npx linkinator --version', { stdio: 'pipe' });

  console.log('Checking links in: ' + servePath);
  execSync('npx linkinator ' + servePath + ' --recurse --skip "localhost|127.0.0.1"', {
    stdio: 'inherit',
    timeout: 120000
  });
...
try {
  execSync('npx pa11y-ci', { stdio: 'inherit', timeout: 120000 });
  ...
  console.error('⚠️  pa11y-ci failed to run');
}
`
}
 ...
 */
function getPrelaunchDependencies(_isPro = false) {
  const deps = {
    linkinator: '^6.0.0',
    'pa11y-ci': '^3.1.0',
  }
