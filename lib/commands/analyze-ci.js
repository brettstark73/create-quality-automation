      ...
      for (const step of job.steps) {
        ...
        if (step.name) {
          ...
          if (stepName.includes('test') || stepName.includes('e2e')) {
            ...
            jobMinutes += 3
          } else if (stepName.includes('install') || stepName.includes('setup')) {
            jobMinutes += 2
          } else {
      ...
      if (hasSteps) {
        ...
        if (hasInstall && !hasCaching) {
          ...
          const savingsPerMonth = Math.ceil(
            savingsPerRun * commitsPerDay * 30
          )
      ...
      if (hasPush && !hasPathFilter && !workflowName.includes('release')) {
        ...
        if (savingsPerMonth > 50) {
          recommendations.push({
            ...
            action: 'Skip CI for docs-only changes (paths-ignore: ["**/*.md", "docs/**"])',
            potentialSavings: savingsPerMonth,
            savingsPerRun: 0,
            priority: 'medium',
          })
...
function generateReport(analysis) {
  ...
  if (optimizations && optimizations.length > 0) {
    ...
    console.log('✅ No optimization opportunities detected - workflows look good!')
    console.log('')
  }
...
async function handleAnalyzeCi() {
  ...
  try {
    ...
    const optimizations = analyzeOptimizations(workflows, commitStats.commitsPerDay)
