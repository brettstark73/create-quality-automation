...
class ProjectMaturityDetector {
  ...
  detect() {
    ...
    if (stats.totalSourceFiles === 0) {
      maturity = 'minimal'
    } else if (stats.totalSourceFiles < 3 && stats.testFiles === 0) {
      maturity = 'bootstrap'
    } else if (
      stats.testFiles > 0 &&
      stats.totalSourceFiles >= 3 &&
    ...
    } else if (stats.totalSourceFiles >= 3) {
      maturity = 'development'
    }
  ...
  hasDocumentation() {
    ...
    // Check for substantial README (> 100 lines)
  ...
  /**
   * Generic file counter with filtering
  ...
  printReport() {
    ...
    console.log(`  • Test files: ${stats.testFiles}`)
    console.log(
      `  • Documentation: ${stats.hasDocumentation ? 'Yes' : 'No'}`
    )
