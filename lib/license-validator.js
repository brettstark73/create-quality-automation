...
class LicenseValidator {
  constructor() {
    ...
    this.licenseDbUrl =
      process.env.CQA_LICENSE_DB_URL ||
      'https://license.aibuilderlab.com/cqa/legitimate-licenses.json'
  }
  ...
  async fetchLegitimateDatabase() {
    try {
      ...
      const response = await fetch(this.licenseDbUrl, {
        ...
        headers: { 'User-Agent': 'create-quality-automation-cli' },
      })
  ...
  async activateLicense(licenseKey, userEmail) {
    try {
      ...
      if (
        !licenseKey.match(
          /^CQA-[A-Z0-9]{4}-[A-Z0-9]{4}-[A-Z0-9]{4}-[A-Z0-9]{4}$/
        )
      ) {
        return {
          success: false,
          error:
            'Invalid license key format. Expected format: CQA-XXXX-XXXX-XXXX-XXXX',
        }
      }
